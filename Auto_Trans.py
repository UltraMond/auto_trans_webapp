...
st.set_page_config(page_title="Auto_Trans v17", layout="wide")
...
with st.sidebar:
    ...
    if api_key:
        try:
            ...
            available_model_ids.sort(reverse=True)
        except Exception as e:
            st.error(f"API 키 확인 필요: {e}")

    target_model_id = st.selectbox("모델 선택", available_model_ids if available_model_ids else ["키 입력 대기"], 
                                   disabled=not available_model_ids)
...
def translate_content(title, desc, srt, target_lang, api_key, model_id):
    ...
    prompt = f"Translate to {lang_name}. Separate sections with '|||'. Format: Title ||| Desc ||| SRT. Keep timecodes.\n\n[Title]: {title}\n[Desc]: {desc}\n[SRT]: {srt}"
    try:
        response = model.generate_content(prompt)
...
def run_translation():
    ...
    if btn_run.button("🚀 번역 시작", type="primary", use_container_width=True):
        ...
        with st.status("🔍 분석 및 번역 중...", expanded=True) as status:
            detected_lang = detect_source_language(u_title, u_srt, api_key, target_model_id)
            st.write(f"💡 감지된 원본 언어: **{detected_lang}** (번역에서 제외)")
